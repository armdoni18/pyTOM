...
def F7_Main_Comp_Force(fem):
    ...
    # ============================================================
    edge_nodes = set(plunger_boundary_edges.flatten())

    connected_air = [
        tri for tri in air_triangles
        if any(node in edge_nodes for node in tri)
    ]
    ...
    # ============================================================
    # PRECOMPUTE centroids (ONLY ONCE → IMPORTANT SPEED UP)
    # ============================================================
    if "centroids" not in fem:
        centroids = np.mean(X[IX[:, 0:3] - 1], axis=1)
        fem["centroids"] = centroids
    else:
        centroids = fem["centroids"]

    # ============================================================
    # Force integration (MST)
    # ============================================================
    for k in range(IntegPathEdges):
        ...
        # slightly shift into air domain
        eps_shift   = 1e-3
        shifted_mid = mid + eps_shift * normal

        # ====================================================
        # FAST nearest element search (VECTORISED)
        # ====================================================
        diff = centroids - shifted_mid
        dist_all = np.sum(diff**2, axis=1)
        closest_e = np.argmin(dist_all)

        # ====================================================
        # Magnetic field at closest element
        # ====================================================
        Bx = fem["Bx"][closest_e]
        By = fem["By"][closest_e]

        # reluctivity ν (consistent with manuscript)
