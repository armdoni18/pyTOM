...
def F0_Main_Mat_Nonlinear(B):
    ...
    Notes
    -----
    The exponent ``b * |B|^2`` is clipped to 700 to avoid float64
    overflow in ``np.exp`` (which overflows near exp(709)). The
    clip threshold corresponds to |B| of about 22 T, which is far
    beyond any physically meaningful operating point for the
    ferromagnetic materials considered here, so this guard has no
    effect in practice.
    """
